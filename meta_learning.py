    ...
    for batch_idx, (inputs, targets) in enumerate(main_train_loader):
        ...
        if batch_idx % params.log_interval == 0:
            ...
            logging.info(
                ...
                f"tot_accuracy {(100.*tot_correct/tot_samples):.3}, "
                f"lr: {cur_lr:.3}"
            )
            if tb_writer is not None:
                ...
                tb_writer.add_scalar(
                    "train_main/tot_accuracy", 100. * tot_correct / tot_samples, params.batch_idx_main
                )
...
def test(
    ...
    if accuracy > params.best_accuracy:
        params.best_epoch = params.batch_idx_main
